...
def main(command,ip_address,username,password, snapshot_count):
    ...
    try:
        ...
        for number in range(0, snapshot_count):
            ...
            for x in result_list:
                ...
                if command_running[6]:
                    index = 6
                    result += show_interface_counters(counters_dict, index, x)
        ...
        to_csv(temp_graph, temp_sensor_names, cpu_graph, memory_graph, date, docker_stats_graph, docker_stats_sensor_names)
        final_result = '\n\n########################################################################## \n\n'
        final_result += min_max_average(temp_graph, temp_sensor_names, cpu_graph, memory_graph, docker_stats_graph, docker_stats_sensor_names)
